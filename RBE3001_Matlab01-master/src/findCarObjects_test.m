...
%process image
balls = findCarObjs(imageOG, Tzero2check, Tcam2check, cameraParams);
...
while ballSize(2) > 0
    ...
    if ballFound == 0
     ...
     if ball.worldCenter(2) ~= 0
        Y = (ball.worldCenter(2) + (.1 * ball.worldCenter(2)))
    end
    overBallAngles = ikinCamera([ball.worldCenter(1) Y 30]);
    ...
    %move down to ball
    ballAngles = ikinCamera([ball.worldCenter(1) Y -40]);
    ...
    %grip ball
    moveGripper(15,pp, SERVO_SERVER_ID);
