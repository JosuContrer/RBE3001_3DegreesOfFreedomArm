...
//float homePosition[3] = {-869.75,-916,2283};
float homePosition[3] = {-865.57, -924.47, 2257.88};
...
int main() {
	 ...
	 *            the DEBUG mahin the main loop is `coms.server()'. This method, which is
	 * invoked at each loop iteration, results in the execution of the `event()'
	 * method on each communication object that was instantiated above
	 * (see Part 2b of source file). The `event()' method implements upstream/downstream
	 * communication between this firmwacro at the beginning of this source file.
	 */
